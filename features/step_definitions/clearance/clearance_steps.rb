...
Given /^(?:I am|I have|I) signed up being "([^\"]+)" (?:as|with) "(.*)\/(.*)"$/ do |type, email, password|
  Factory(type.to_sym,
          :email                 => email,
          :password              => password,
...
Given /^a user "([^"]*)" exists without a salt, remember token, or password$/ do |email|
  MongoMapper.database["users"].insert({"username" => "user", "email" => "#{email}"})
  #user = Factory(:user, :email => email)
  #MongoMapper.database["users"].update({"_id" => user["_id"]}, {"$set" => {"salt" => null, "encrypted_password" => null, "remember_token" => null}})
...
Given /^(?:I am|I have|I) signed in being "([^\"]+)" (?:with|as) "(.*)\/(.*)"$/ do |type, email, password|
  Given %{I am signed up being "#{type}" as "#{email}/#{password}"}
